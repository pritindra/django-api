...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            engine = create_engine('snowflake://vickydeka:Vicky1213@GVNUKEQ-WR96016/new_new?warehouse=COMPUTE_WH&schema=new_new')
            metadata = MetaData(bind=engine)
            employee_table = Table('employee', metadata,
                                   # Column('id', Integer, primary_key=True),
                                    Column('name', String),
                                    Column('position', String),
                                    Column('departmenet', String))
            employee_table.create(checkfirst=True)
            df = pd.read_csv(r"C:\Users\nmedh\Downloads\rest_api-master\samsung\employees\management\commands\employee.csv")
            df.columns = map(lambda x: str(x).upper(), df.columns)
